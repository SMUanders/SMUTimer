...
> Status: **SMU Tid v2.1 er PRODUCTION-LIVE på https://smutimer.netlify.app** (produktversion = `package.json`, vist diskret "SMU Tid v2.1"). Custom-domain (`tid.smu.signmeup.dk`) + TLS + Supabase Auth URL + **SSO-cutover er PENDING** (ikke live endnu). Hub peger fortsat på `smutimer.netlify.app`.
> v2.1 tilføjede: pause fra aktiv opgave med automatisk genoptag · read-only detaljevisning i "Min dag" · synlig produktversion · SMU-sagsvælger (typeahead mod OS-ejet read-contract; QR-klart fundament).
...
- **Pause** tæller **ikke** som arbejdstid. **Pause fra aktiv opgave (v2.1):** en normal aktiv opgave kan sættes på pause direkte — opgaven lukkes til historik frem til pausestart, huskes (`pauseContext`, localStorage som help/omgøring), og **genoptages automatisk** efter pausen ("Genoptag [reference]") med bevaret reference/kategori/aktivitet/note. Standalone pause fra "Hvad nu?" (uden underliggende opgave) går tilbage til "Hvad nu?". Ingen nested pause i hjælp/omgøring. Ingen DB-ændring.
...
- **Min dag** er en **read-only** lodret tidslinje (arbejde/hjælp/omgøring/pause/fravær/hul/overlap). Ingen Rediger/Slet/Ny/Udfyld på medarbejderskærmen. **Detaljevisning (v2.1):** klik/tap på egen afsluttet registrering åbner en detalje (tid/varighed/kategori/aktivitet/ordre-sag/note/omgøring) via `EntryDetail` + `DayTimeline.onSelectEntry`. Alt er read-only **undtagen noten på ens EGEN registrering** (v2.1.1): "Rediger note" gør kun note-feltet redigerbart (Gem note/Annuller), sender kun `note`-feltet til `updateEntry` (ikke hele entryen), og RLS owner-scope (`employee_id = auth.uid()`) sikrer at man kun kan redigere egne. Ingen ændring af tid/kategori/aktivitet/sag/omgøring/employee_id. Andres registreringer forbliver read-only her; leder/admin-korrektion sker uændret i LeaderDay.
- **SMU-sag (v2.1):** SMU OS er **autoritativ ejer** af sager og SMU-numre (`public.sager`: `id` uuid PK · `smu_nummer` unik · `titel` · `kunde_id → kunder.navn`). Tid **refererer** kun — ingen parallel sagssandhed. Medarbejderen kan i "SMU-nr. / sag"-feltet søge eksisterende sager (typeahead, `SagPicker`, debounced) via en **OS-ejet read-contract** `public.tid_sag_search(q)` (SECURITY DEFINER, kræver `har_app_adgang('tid')`, kun ikke-slettede/aktive sager, kun minimale felter; ingen bred `os`-adgang, ingen anon-adgang). Ved valg gemmes en **stabil blød reference** (`sag_id` uuid, INGEN FK) + et **display-snapshot** (`sag_smu_nummer`) på `tid_time_entries`/`tid_current_tasks` — så historik er læsbar uden OS-adgang. **Fritekst bevares** til ikke-sagsbundet arbejde (morgenmøde, rengøring, vedligehold m.m.); SMU-sag er **ikke** obligatorisk. Referencen bevares gennem start→afslut og suspend→resume (hjælp/pause/omgøring). Hub-migration `20260925120001`. **QR** bygges ikke nu, men fundamentet er QR-klart: en QR-identifikator kan senere resolve til samme `sag_id` via samme contract/model.
- **Synlig version (v2.1):** produktversionen har **`package.json` som single source of truth** (injiceret ved build som `__APP_PRODUCT_VERSION__`), vist diskret som "SMU Tid v2.1". Build-tidsstempel (`__APP_VERSION__` + `version.json`) er teknisk build-id til update-detektion (UpdateBanner) og fejlsøgning (tooltip) — adskilt fra produktversionen.
